<#
    .SYNOPSIS
    Retrieves the specified account from CyberArk, Launches RDP and Copies the Password to the clipboard.

    .DESCRIPTION
    This script allows the user to Check-Out / Retrieve the password for the specified account from CyberArk
    and stores the password into the clipboard while launching RDP.

    The script does require the use of an external EXE for retrieval of the SAML response if using SAML Auth.
    ...
    .PARAMETER UserSafe
    [string]:  This is the target credentials safe.

    .PARAMETER UserName
    [string]:  This is the target credentials username.

    .PARAMETER UserAddress
    [string]:  This is the target credentials address.

    .PARAMETER ServerAddress
    [string]:  This is the target server address.

    .PARAMETER OutputFile
    [string]:  This is the file to write the RDP information to.
...
[CmdletBinding()]
    param(
        ...
        [string] $UserAddress = "192.168.184.128",

        [Parameter(
            Mandatory = $false,
            ValueFromPipeline = $true
            )]
        [string] $ServerAddress = "192.168.184.128",

        [Parameter(
            Mandatory = $false,
            ValueFromPipeline = $true
            )]
        [string] $OutputFile = "!_Test_.rdp"
    )
...
function Get-Accounts
{
    ...
    # Build the hash table to hold the web request properties.
    $_requestAttributes = @{
        URI = (Join-Url -Path $_baseURL -ChildPath $_newQueryStub)
        Method = "Get"
        WebSession = $WebSession
        #Headers = $_Headers
        #Body 	= ($_body | ConvertTo-Json)
...
function Get-AccountSecret
{
    ...
    # Get the target URL.
    $_targetURL = $ACCOUNTACTIONSURLS["GetPasswordValue"]
...
function Write-RDPFile
{
    ...
    session bpp:i:32
    winposstr:s:0,1,577,27,2513,1266
    compression:i:1
    keyboardhook:i:2
    ...
    $rdpAddress = ("`r`nfull address:s:{0}" -f $Address)

    # Build the username line
    $rdpUsername = ("`r`nusername:s:{0}" -f $Username)

    # Join the address and username to the file
    $rdpFile += $rdpAddress
    $rdpFile += $rdpUsername

    #endRegion RDP File
    Out-File -FilePath $Path -InputObject $rdpFile
}
...
$LoggedOnUserUserName = "Place Holder"
...
# Search for any accounts that match the UserSafe, UserName, and UserAddress.
$reqAttributes = @{
    "BaseURL" = $_baseURL
    "Token" = $_authToken
    "SearchText" = ("{0} {1}" -f $UserName, $UserAddress)
    "SafeName" = $UserSafe
}
$FoundAccounts = Get-Accounts @reqAttributes

# Check the number of accounts found.
if ($FoundAccounts.Count -eq 0)
{
    Write-Warning ("No Accounts found for User ({0}) in Safe ({1}) with Username ({2}) and Address ({3})" -f $LoggedOnUserUserName, $UserSafe, $UserName, $UserAddress)
}
elseif ($FoundAccounts.Count -gt 1)
{
    Write-Warning ("No Accounts found for User ({0}) in Safe ({1}) with Username ({2}) and Address ({3})" -f $LoggedOnUserUserName, $UserSafe, $UserName, $UserAddress)
}
else
{
    ...
    $_foundCredential = $FoundAccounts.Value[0]

    # Get the password.
    $reqAttributes = @{
        "BaseURL" = $_baseURL
        "Token" = $_authToken
        "AccountID" = $_foundCredential.Id
    }
    $_credPassword = Get-AccountSecret @reqAttributes

    # Create the RDP file.
    Write-RDPFile -Path $OutputFile -Address $ServerAddress -Username $UserName

    # Copy the password to the clipboard.
    Set-Clipboard -Value $_credPassword

    # Launch RDP.
    Start-Process "$env:windir\system32\mstsc.exe" -ArgumentList $OutputFile

    # Do Something
    Write-Host "Be Safe Out There!"
}
