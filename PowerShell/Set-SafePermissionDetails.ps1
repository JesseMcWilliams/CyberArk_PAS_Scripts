<#
    .SYNOPSIS
    This script will set or remove the detailed permissions specified in the input file(s).
    ...
    The input CSV file needs to have the following header.
        SafeName , UserType , Username ,Domain , Action , Individual Permissions...
        ...
        Permissions:  This is the individual permissions that can be set.  Each permission is a separate header with a value of
                      True or False.
                useAccounts
                retrieveAccounts
                listAccounts
                ...
                viewAuditLog
                viewSafeMembers
                requestsAuthorizationLevel1  <-- Only Level 1 or Level 2 can be set.  Not both.
                requestsAuthorizationLevel2  <-- Only Level 1 or Level 2 can be set.  Not both.
    ...
    [hashtable]:  A Hash Table containing the permissions to be set or removed..
    ...
    .\Set-SafePermissionDetails.ps1 -Credential $MyCred

    .EXAMPLE
    In this example all required arguments are specified on the command line.

    .\Set-SafePermissionDetails.ps1 -PVWAURL "https://epv.company.com" -InputFiles @("Input_Safe_Permissions_1.csv","Input_Safe_Permissions_2.csv")
    ...
    Create a DOC/Batch file name:  Set-SafePermissionDetails.bat
    Contnets:
        CLS
        @Echo Off
        ...
        PowerShell -NoProfile -ExecutionPolicy Bypass -File ".\Set-SafePermissionDetails.ps1" ^
...
[CmdletBinding()]
    param(
        ...
        [string] $TemplateSafe = "Z_Permission_Template_Safe",
...
function Import-SafePermissions
{
    ...
    foreach ($_file in $Files)
    {
        ...
        foreach ($_line in $_uifCSV)
        {
            ...
            # Create the domain user variable.
            ...
            # Write status line
            Write-Host ("{0} : Row ({1:d4}):  Safe ({2}) : User ({3}) : Action ({4:f6})" -f (Get-Date -Format "yyyy-MM-dd HH:mm:ss"), $_csvLoopCount, $_line.SafeName, $domainUserName, $_line.Action)
            ...
            # Check if the Safe URL ID was retrieved.
            if ($null -ne $_targetSafe)
            {
            ...
            {
                Write-Warning ("{0} : Safe Not Found:  {1}" -f (Get-Date -Format "yyyy-MM-dd HH:mm:ss"), $_line.SafeName)
                $_messageAction = "Add Permissions - Safe Not Found!"
                $_actionResultSuccess = $false
            }
