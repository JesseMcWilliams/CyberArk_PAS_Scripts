<#
    .SYNOPSIS
    This script will set Change, Verify, or Reconcile on the accounts specified in the input file(s).
    
    .DESCRIPTION
    This script will process one or more CSV files and perform the requested action on the account.
    
    The input CSV file needs to have the following header.  The SafeName, Address, and Username
    identifies the account to take the action on.  The combination must be unique.
        SafeName, Address, Username, Action, Force
    
        SafeName: This is the name of the safe where the account lives.

         Address:  This is the address specified on the account.

        UserName: This is the Username of the account.

          Action:  Change / Reconcile / Verify

           Force:  If disabled by user, override.
    ...
    .EXAMPLE
    This example is the most basic.  This will show help information.

    .\Set-AccountAction.ps1
    ...
    the credentials.  The user needs to have updated the PVWAURL in the script.

    $MyCred = Get-Credential
    .\Set-AccountAction.ps1 -Credential $MyCred
    ...
    Create a DOC/Batch file name:  Set-AccountAction.bat
    Contnets:
        @Echo Off    
        CLS
...
[CmdletBinding()]
    param(
        ...
        [string] $PVWAURL = "https://192.168.184.132",
        #[string] $PVWAURL = "https://192.168.184.132",
        ...
        #[pscredential] $Credential = (New-Object System.Management.Automation.PSCredential ("CA_Jesse", (ConvertTo-SecureString "Password22!" -AsPlainText -Force))),
        ...
        [string[]] $InputFiles
        #[string[]] $InputFiles = @("C:\Engagements\JesseMcWilliams\CyberArk_PAS_Scripts\PowerShell\!_Input_Set-AccountAction.csv")
    )
...
function Invoke-PASRestMethod 
{
	...
	Begin {
        ...
        $_RequestRawResponse = $false
        if ($ReturnRawResponse) {

			$null = $PSBoundParameters.Remove("ReturnRawResponse")
            $_RequestRawResponse = $true
...
##########################################################
function Request-AccountAction
{
    [CmdletBinding()]
    param(
        [Parameter(
    ...
    if ($Action -ieq "Verify")
    {
        ...
        # Verify Action
        $_resultObject = Set-AccountVerify @PSBoundParameters
        
    }
    elseif ($Action -ieq "Change")
    {
        # Change Action
        $_resultObject = Set-AccountChange @PSBoundParameters
    }
    elseif ($Action -ieq "Reconcile")
    {
        # Reconcile Action
        $_resultObject = Set-AccountReconcile @PSBoundParameters
    }
    else
    {
        # Invalid Action
        # Set the Action Result.
        $_resultObject.Message = ("Invalid Action:  {0}" -f $_line.Action)
    }

    # Return the result object.
    return $_resultObject
}
function Set-AccountVerify
{
    [CmdletBinding()]
    param(
        [Parameter(
    ...
    # Get the Method and Stub URL.
    $_stubInfo = $ACCOUNTACTIONSURLS["VerifyCredentials"]

    # Get the target URL.
    $_targetURL = $_stubInfo["URL"]
    ...
    Write-Verbose ("{0} : {1} : URL : {2}" -f (Get-Date -Format "yyyy-MM-dd HH:mm:ss"), $Action, $_newQueryStub)
    
    # Build the hash table to hold the web request properties.
    $_requestAttributes = @{
        URI = (Join-Url -Path $BaseURL -ChildPath $_newQueryStub)
        Method = $_stubInfo["Method"]
        WebSession = $WebSession
    ...
    {
        # Result is not null.
        if (($_result.StatusCode -ge 200) -and ($_result.StatusCode -lt 300))
        {
            # Assume success
            $_resultObject["IsSuccess"] = $true
            $_resultObject["Message"] = ("Action ({0}) : ID ({1})" -f $Action, $AccountID)
        }
    }
...
function Set-AccountChange
{
    ...
    # Get the Method and Stub URL.
    $_stubInfo = $ACCOUNTACTIONSURLS["ChangeCredentialsImmediately"]
...
function Set-AccountReconcile
{
    ...
    # Get the Method and Stub URL.
    $_stubInfo = $ACCOUNTACTIONSURLS["ReceoncileCredentials"]
...
function Set-AccountProperties
{
    [CmdletBinding()]
    param(
        [Parameter(
    ...
    # Get the Method and Stub URL.
    $_stubInfo = $ACCOUNTSURLS["UpdateAccount"]
    ...
    # Build the body.
    $_body = ($Properties | ConvertTo-Json)

    # If there is only one property being set it still needs to be sent as an array.
    # The ConvertTo-Json will not do this if it is only one item.  Manually add square brackets.
    ...
    if ($null -ne $_result)
    {
        ...
        if ($_result -ne "")
        {
            # Assume success.  This isn't the best way.  
            # It can say success and the properties were not updated.
            $_resultObject["IsSuccess"] = $true
...
function Import-AccountActions
{
    [CmdletBinding()]
    param(
        [Parameter(
    ...
    foreach ($_file in $Files)
    {
        ...
        Write-Host ("{0} : Rows found in file:  {1}" -f (Get-Date -Format "yyyy-MM-dd HH:mm:ss"), $_uifCSV.count)
        ...
        foreach ($_line in $_uifCSV)
        {
            ...
            # Write status line
            Write-Host ("{0} : Row ({1:d4}):  SafeName ({2}) : Address ({3}) : Username ({4}) : Action ({5})" -f (Get-Date -Format "yyyy-MM-dd HH:mm:ss"), $_csvLoopCount, $_line.SafeName, $_line.Address, $_line.Username, $_line.Action)

            # Set the Account properties to find.
            $_requestAttributes = @{
                BaseURL = $BaseURL
                SafeName = $_line.SafeName
                SearchText = ("{0} {1}" -f $_line.Username, $_line.Address)
            }
            $_accountResults = Get-Accounts @_requestAttributes

            Write-Host ("{0} : All Accounts Found : {1}" -f (Get-Date -Format "yyyy-MM-dd HH:mm:ss"), $_accountResults.count)

            # Filter the results again.  The previous search returns the result if the username and address are found
            # anywhere in the account properties.
            [object[]]$_filteredResults = $_accountResults.Value | Where-Object {$_.userName -ieq $_line.Username -and $_.address -ieq $_line.Address}
            
            # Status of accounts remaining.
            Write-Host ("{0} : Filtered Accounts  : {1}" -f (Get-Date -Format "yyyy-MM-dd HH:mm:ss"), $_filteredResults.count)

            # Only take action if only 1 account remains.
            if ($_filteredResults.Count -eq 0)
            {
                # Set the Action Result.
                $_actionResultSuccess = "No Accounts after Filter."
            }
            elseif ($_filteredResults.Count -eq 1)
            {
                # Set the Action Message.
                $_messageAction = $_line.Action

                # Build the request attribute.
                $_requestAttributes = @{
                    BaseURL = $BaseURL
                    AccountID = $_filteredResults[0].id
                    Action = $_line.Action
                }
                ...
                # Assign the account CPM status to variables.
                $_targetAccount_SecretManagement = $_filteredResults[0].secretManagement
                $_targetAccount_CPMEnabled = $_targetAccount_SecretManagement.automaticManagementEnabled
                $_targetAccount_CPMReason = $_targetAccount_SecretManagement.manualManagementReason

                # Check if the Account Management is Enabled
                if ($_targetAccount_CPMEnabled)
                {
                    # CPM Enabled.  Request the action.
                    $_actionResult = Request-AccountAction @_requestAttributes
                }
                else
                {
                    # CPM Disabled.  By CPM or User?
                    if ($_targetAccount_CPMReason -ilike "(CPM)*")
                    {
                        # Disabled by CPM.  Request the action.
                        $_actionResult = Request-AccountAction @_requestAttributes
                    }
                    else
                    {
                        # Disabled by user.  Is Force set to True?
                        if (($_line.Force -ieq "Yes") -or ($_line.Force -ieq "True"))
                        {
                            # Force has been requested.  Clear reason and enable management.
                            # Build the properties hashtable.
                            $_newProperties = @(
                                @{
                                    op = "Replace"
                            ...
                            <#
                                @{
                                    ...
                                    value = @{automaticManagementEnabled = "true"}
                                },
                                @{
                                    op = "Remove"
                            ...
                            #>
                            $_clearReason = Set-AccountProperties @_requestAttributes -Properties $_newProperties

                            
                            # If the Clear Reason and enable CPM was successfull.  Set Action.
                            if ($_clearReason.IsSuccess)
                            {
                                # Request the action.
                                $_actionResult = Request-AccountAction @_requestAttributes
                            }
                        ...
                        {
                            # Force was not requested.  Do nothing.
                            $_actionResultSuccess = ("CPM Disabled by User:  {0}" -f $_targetAccount_CPMReason)
                        }
                    }
                }

                # Test the Action Result.
                if ($null -eq $_actionResult)
                {
                    #  Result is Null.  This is a failure.
                }
                else
                {
                    ...
                    {
                        # Success
                        $_actionResultSuccess = ("Success : {0}" -f $_actionResult.Message)
                    }
                    else
                    {
                        # Failed
                        $_actionResultSuccess = ("Failed : {0}" -f $_actionResult.Message)
                    }
                }
                
            }
            elseif ($_filteredResults.Count -gt 1)
            {
                # Set the Action Result.
                $_actionResultSuccess = ("Too Many Accounts after Filter.  Found ({0})" -f $_filteredResults.Count)
            }
...
if (($null -ne $CyberArkAuthToken) -and ($CyberArkAuthToken -ne ""))
{
    ...
    Import-AccountActions -BaseURL $_baseURL -Files $InputFiles -AuthToken $CyberArkAuthToken
}
else
{
    # Write error and exit
